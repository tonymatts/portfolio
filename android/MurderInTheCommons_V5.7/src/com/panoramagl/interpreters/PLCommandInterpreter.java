/*MODIFIED BY ADDING CUSTOMIZED METHODS BEGINNING AT LINE 120 FOR Murder in the Commons
 * 
 * PanoramaGL library
 * Version 0.2 beta
	...
	protected void parseCommands(List<PLIToken> tokens, int tokenIndex)
	{
		if(tokenIndex < tokens.size())
		{
			...
			if(token.getType() == PLTokenType.PLTokenTypeFunction)
			{
				...
				}else if(fx.equals("startActivity"))
				{
				/***********************************************************
				 ******** Change for adding our own methods ****************
				 *************************************************************/
				 
					//gets the parameter of startActivity function
					tokenIndex = this.parseFunction(tokens, tokenIndex, tokenInfo, PLTokenType.PLTokenTypeString.ordinal());
					//write a log
					Log.i("HELLO","Hello started");
					// get the current context
					Context context = MyApplication.getContext(); 
					//call the method for the TestActivity parameter
					if(tokenInfo.getString(0).equals("TestActivity")){
						//runs TestActivity
						PuzzleMethods.TestActivity(context);
					}//else ... add HERE your other methods
					else if(tokenInfo.getString(0).equals("PhoneBookMain"))
					{
						PuzzleMethods.PhoneBookMain(context);
					}
					else if(tokenInfo.getString(0).equals("ShiftingMap"))
					{
						PuzzleMethods.ShiftingMap(context);
					}
					else if(tokenInfo.getString(0).equals("Envelope"))
					{
						PuzzleMethods.Envelope(context);
					}
					else if(tokenInfo.getString(0).equals("GhostLeg0"))
					{
						PanoramaGLActivity.flierNumber=0;
						PanoramaGLActivity.flierOpen[0]=true;
						PuzzleMethods.GhostLeg(context);
					}
					else if(tokenInfo.getString(0).equals("GhostLeg1"))
					{
						PanoramaGLActivity.flierNumber=1;
						PanoramaGLActivity.flierOpen[1]=true;
						PuzzleMethods.GhostLeg(context);
					}
					else if(tokenInfo.getString(0).equals("GhostLeg2"))
					{
						PanoramaGLActivity.flierNumber=2;
						PanoramaGLActivity.flierOpen[2]=true;
						PuzzleMethods.GhostLeg(context);
					}
					else if(tokenInfo.getString(0).equals("GhostLeg3"))
					{
						PanoramaGLActivity.flierNumber=3;
						PanoramaGLActivity.flierOpen[3]=true;
						PuzzleMethods.GhostLeg(context);
					}
				...
				throw new RuntimeException("parseCommands expected a valid command, not "+fx);
		}
	}
