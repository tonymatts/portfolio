...
def mysql(database_name, hostname, username, password):
...
if len(sys.argv) < 2 or not regex.match(sys.argv[1]):
    print 'USAGE:\n\n    extract_mysql_models.py username:password@hostname/database\n\n'
else:
    m = regex.match(sys.argv[1])
    print mysql(m.group(4), m.group(3), m.group(1), m.group(2))
